...
void SensorModelEvaluator::calcRangeTable()
{
  ...
  for ( auto &exp_it : expected_meas_ )
  {
    auto obs_it = observed_meas_.find( exp_it.first );
    if ( obs_it != std::end( observed_meas_ ))
    {
      auto p_obs = obs_it->second;
      auto p_exp = exp_it.second;
      
      double obs_range = cv::norm( origin_laser - p_obs.cv());
      double exp_range = cv::norm( origin_laser - p_exp.cv());
      
      ranges_exp_obs_.insert( std::make_pair( exp_it.first, std::make_pair( exp_range, obs_range )));
    }
  }
}
