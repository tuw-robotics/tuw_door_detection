...
protected:
	...
	void plot(const std::vector<DoorDetection> &_detections);
...
private:
	...
	bool structureMode(const sensor_msgs::LaserScan &_laser, std::vector<DoorDetection> &_detections);
	bool kernelMode(const sensor_msgs::LaserScan &_laser, std::vector<DoorDetection> &_detections);
};
}
