...
bool DoorDepthDetector::contourMode(const sensor_msgs::LaserScan &_scan) {
    ...
    cv::Mat img = cv::Mat::zeros(map.height(), map.width(), CV_8UC3);
    cv::Mat corner_img = cv::Mat::zeros(map.height(), map.width(), CV_8U);
    ...
    for (const auto &elem : contours) {
        ...
        elem->render(map, img, color, 2, true);
    }
    if (potential_doors) {
...
bool DoorDepthDetector::structureMode(const sensor_msgs::LaserScan &_scan, std::vector<DoorDetectionPtr> &_detections) {
...
bool DoorDepthDetector::processLaser(const sensor_msgs::LaserScan &_scan) {
    ...
    std::vector<DoorDetectionPtr> detections;
    ...
    //t = tf_laser2world * t;
    for (DoorDetectionPtr d : detections) {
        if (!d->validDetection()) {
            //TODO: cleanup
            continue;
        }
